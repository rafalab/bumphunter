...
bumphunterEngine <- function(mat, design, chr=NULL, pos, cluster=NULL,
    ...
    ## tmp <- abs(cbind(rawBeta, permRawBeta))
    ## chunksize <- ceiling(nrow(tmp)/workers)
    ## pvs <- foreach(subMat=iter(tmp, by="row", chunksize=chunksize),
    ##                .combine="c", .packages = "bumphunter") %dorng% {
    ##                    (B+1-rowRanks(subMat)[,1]) / B
    ##                } 
    ## attributes(pvs)[["rng"]] <- NULL
